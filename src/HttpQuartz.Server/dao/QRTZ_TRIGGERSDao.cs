...
namespace HttpQuartz.Server.dao
{
    ...
    public class QRTZ_TRIGGERSDao : BaseDao<QRTZ_TRIGGERS>
    {
        ...
        public QRTZ_SIMPROP_TRIGGERS SinglePropTrigger(string schedName, string triggerName, string triggerGroup)
        {
            using var db = Open();
            return db.SingleWhere<QRTZ_SIMPROP_TRIGGERS>(DictBuilder
                .Assign("SCHED_NAME", schedName)
                .Assign("TRIGGER_NAME", triggerName)
                .Assign("TRIGGER_GROUP", triggerGroup)
            );
